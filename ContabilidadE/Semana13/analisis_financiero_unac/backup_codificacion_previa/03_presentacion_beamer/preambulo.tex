...
\begin{tikzpicture}[node distance=0.12cm, every node/.style={font=\tiny}]
  ...
  \node[caja, right=of b] (c) {Analisis\\financiero};
  \node[caja, right=of c] (d) {Decision\\empresarial};
...
\begin{tikzpicture}[font=\tiny, scale=0.88, transform shape]
  ...
  \node[draw=BloqueMorado, fill=purple!8, rounded corners=2pt, align=center, minimum width=2.7cm] (d) at (1.7,-1.65) {Decision\\mejor informada};
  \draw[-{Latex[length=2mm]}, thick, gray] (h) -- (d);
  \draw[-{Latex[length=2mm]}, thick, gray] (v) -- (d);
\end{tikzpicture}}

\title[Analisis financiero]{Analisis financiero:\\analisis horizontal y analisis vertical de estados financieros}
